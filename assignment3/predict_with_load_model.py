...
def show_input_and_prediction(image_number,predictions,x_train,y_train,epoch=0):
	...
	# plt.show()
	output_dir = './predictions_validation512/'
...
# data_dir = '/data/s1546449/maps_data_validation200/' # validation set, Leiden & Amsterdam
data_dir = '/data/s1546449/maps_data_validation512/' # validation set, Leiden & Amsterdam

# data_dir = '/data/s1546449/maps_data_28_zoom12/'

# Load the data
amount_of_examples = int(200) # Load only 2300 images for dataset maps_data_28_zoom12
# x_train, y_train, x_test, y_test = load_data(amount_of_examples,data_dir)
x_train, y_train = load_data(amount_of_examples,data_dir)
...
print(x_train.shape[0], 'validation samples')
...
dimensionality = x_train.shape[1]*x_train.shape[2]*x_train.shape[3]

model = load_model('./archive/network_cnn_512.h5')
# model = load_model('archive/network_cnn_200_testing.h5') 


predictions = model.predict(x_train)
# predictions_test = model.predict(x_test)
for i in range(len(x_train)):# x_train is actually validation set in this sense
	show_input_and_prediction(i,predictions,x_train,y_train,i)
