...
from keras.layers import Dense, Dropout, Conv2D, Input
...
figsave_dir = './predictions_cnnmlp_test/'
...
x = Conv2D(16,kernel_size=(5,5),padding='same',activation='relu')(input_img)

x = Conv2D(8,kernel_size=(5,5),padding='same',activation='relu')(x)

x = Conv2D(3,kernel_size=(5,5),padding='same',activation='sigmoid')(x)

# The CNN part of the model
model1 = Model(input_img, x)

dimensionality = 784*3

input_img2 = Input(shape=(dimensionality,))

y = Dense(dimensionality//(8), activation='relu')(input_img2)

y = Dense(dimensionality, activation='sigmoid')(y)

model2 = Model(input_img2, y)
...
plt.legend()
plt.savefig(figsave_dir + 'loss.png')

print ('Saving Model...')
model.save('./archive/network_CNNMLP_testing.h5')
